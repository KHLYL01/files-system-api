...
public class OutBoxServiceBackground {
    ...
    public void getAllEvents() {
        ...
        for (OutBox box : outBoxes) {
            try {
                if (box.getEvent_type().equals(EventTypes.SENT_NOTIFICATION_TO_USER)) {
                    notificationService.sendNotificationToUser(objectMapper.readValue(box.getPayload(), NotificationRequest.class));
                } else if (box.getEvent_type().equals(EventTypes.SENT_NOTIFICATION_TO_ALL_MEMBERS)) {
                    notificationService.sendNotificationToAllMembers(objectMapper.readValue(box.getPayload(), NotificationRequest.class));
                } else if (box.getEvent_type().equals(EventTypes.COMPARE_FILES)) {
